...
public class Delete extends JPanel {
	...
	public Delete(CardLayout layout, JPanel contentPane, File file_produse) {
		setLayout(null);
							...
							if (p[0].equals(produs.getDenumire()) && p[1].equals(produs.getCategorie())) {
								...
								for (int i = 2; i < p.length; i++) {
									if (produs.getTaraOrigine().equals(tari.get(i-2))) {
										...
										if (Double.parseDouble(p[i]) != 0.0)
											stergereProdus = false;
										lineReplace += " " + p[i];
									}
									produsComp += " " + p[i];
								}
